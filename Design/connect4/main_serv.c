...
//file descripter for push button, frame bufer, fnd, led
int ps_fd, fb_fd, fnd_fd, led_fd; 
//file descripter for tcp/ip connection
int serv_sock, clnt_sock;

// send/recv data string
char buffer[2];	
...
int mode; //game mode - single/multi1/multi2
unsigned int breakpoint; //flag for reset

unsigned char pushed_value; // push button input value
unsigned int pushed_num; //save pushed number

unsigned char led_seg_data; //led save value
...
static void sig_handler(int signo); // signal handler to break from accept() in tcp connection
...
int main(int argc, char* argv[]) {
    ...
    int maxVal;	 //coordination for highest weight
    ...
    ps_fd = open(PSDEVFILE, O_RDONLY, O_SYNC);
    if(ps_fd < 0) {
    ...
    sigact.sa_flags = 0;
    //if signal is received, raise interrupt
    sigact.sa_flags |= SA_INTERRUPT;

    //register SIGALRM in the sigaction list
    ...
    while(1) { 
       ...
       //if '0' of connect4 logo is pressed, start minigame
	...
	{
	    // display multiplay selection - one board / two board
	     for(i = 274; i < 360; i++) {
		for(j = 270; j < 557; j++)  {
		...
		setsockopt(serv_sock, SOL_SOCKET, SO_REUSEADDR, 0, 0);
	...
	while(1) {
	    // case of 1p->user, 2p oneboard->all, 2p twoboard->server(this board's player)
	    if(turn == RED || mode == 2 || (mode == 3 && turn == RED))
	    {
		//color the side with player's color
		if(mode == 2 || mode == 3) {
		    a = 0;
		    r = 255;
		    g = (turn == RED) ? 0 : 255;
		...
		// send x position to the client if twoboard mode
		if(mode == 3) {
		    // save the x position in the buffer
		   sprintf(buffer, "%d", xpos);
		   if(send(clnt_sock, buffer, sizeof(buffer), 0) == -1)
		   {
		       printf("send() error.\n");
		       exit(1);
		   }
		...
		insert(table, xmax, turn); 
		ypos = ymax;
		xpos = xmax;
	    }
	    
	    //if 2p twoboard -> client's turn(opponent board)
	    if(turn == YELLOW && mode == 3) {
		...
		if((recv(clnt_sock, buffer, 2, 0)) < -1) {
		    printf("recv() error.\n");
		    exit(1);
		}
		xpos = atoi(buffer);
...
//empty function for alarm signal handling
...
void hidden_piece()
{
	...
	//turn on eight leds sequencially, randomly
	for(i = 0; i < 8; i++) {
	...
	//wait until 'proceed' text is pressed
	do{
	   ts_read(&xcrd, &ycrd);
